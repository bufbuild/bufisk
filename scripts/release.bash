...
if [ -z "${INSIDE_DOCKER}" ]; then
  ...
  echo "${RELEASE_MINISIGN_PRIVATE_KEY_PASSWORD}" | minisign -S -s "${secret_key_file}" -m .build/release/bufisk/assets/sha256.txt
  set -x
  exit 0
fi
...
for os in Darwin Linux Windows; do
  for arch in x86_64 arm64; do
    ...
    CGO_ENABLED=0 GOOS=$(goos "${os}") GOARCH=$(goarch "${arch}") \
      go build -a -ldflags "-s -w" -trimpath -buildvcs=false -o "${dir}/bin/${binary}${extension}" "${DIR}"
